...
const FunctionToken = React.forwardRef<HTMLElement, ITokenComponentProps>(
  ({ bindingId, tokenIndex }, ref) => {
    ...
    const groupedOptions = filteredFunctions.reduce(
      ...
      {} as Record<string, { meta: IFunctionMetadata; index: number }[]>,
    );

    function handleSelectName(meta: IFunctionMetadata) {
      setSearch("");
      setSelectingName(false);
      if (meta.args.length) {
        setSelectedArgIndex(
