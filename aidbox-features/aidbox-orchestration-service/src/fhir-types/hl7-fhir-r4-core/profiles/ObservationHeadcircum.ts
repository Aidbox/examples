...
export interface observation_headcircum extends Observation {
    category: CodeableConcept[];
    subject: Reference<"Patient">;
}

export type Observation_headcircum_Category_VSCatSliceInput = Omit<CodeableConcept, "coding">;
...
export class Observation_headcircumProfile {
    ...
    toProfile () : observation_headcircum {
        return this.resource as observation_headcircum
    }

    public setVscat (input?: Observation_headcircum_Category_VSCatSliceInput): this {
    ...
    public getVscat (): Observation_headcircum_Category_VSCatSliceInput | undefined {
        const match = {"coding":{"code":"vital-signs","system":"http://terminology.hl7.org/CodeSystem/observation-category"}} as Record<string, unknown>
        const list = this.resource.category
        if (!list) return undefined
        const item = list.find((item) => matchesSlice(item, match))
        if (!item) return undefined
        return extractSliceSimplified(item as unknown as Record<string, unknown>, ["coding"]) as Observation_headcircum_Category_VSCatSliceInput
    }
